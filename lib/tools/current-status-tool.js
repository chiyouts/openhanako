...
export function createCurrentStatusTool(deps = {}) {
  ...
  return {
    ...
    description:
      "Lightweight current-environment status. Use it to check fresh state such as time, logical date, agent identity, model identity, and the user's visible UI context. IMPORTANT: The date/time shown in the system prompt is a snapshot from when the session was created and may be outdated. For any time-sensitive operation (scheduling, reminders, date calculations, greetings, or when you believe knowing the precise current time matters), always call this tool with key=\"time\" or key=\"logical_date\" first to get the accurate current time. When the user refers to this file, here, the current/open/visible/selected item, the pinned window, or the current folder, call this tool first to resolve the reference.",
