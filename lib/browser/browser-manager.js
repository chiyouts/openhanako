...
export class BrowserManager {
  constructor() {
    ...
    this._sessionPath = null; // 当前绑定的 sessionPath（launch 时写入，close 时清空）
  ...
  getBrowserSessions() {
    const state = this._loadColdState();
    // 合入当前活跃的
    const session = this._sessionPath || this._getCurrentSession();
  ...
  async launch(sessionPath) {
    ...
    const sp = sessionPath || this._getCurrentSession();
    await this._sendCmd("launch", { sessionPath: sp, headless: this._headless });
    this._running = true;
    this._sessionPath = sp;
  ...
  async navigate(url, sessionPath) {
    ...
    // 更新冷保存
    const session = sessionPath || this._sessionPath || this._getCurrentSession();
  ...
  async scroll(direction, amount = 3) {
    ...
    this._url = result.currentUrl || this._url;
    return result.text;
  }
