...
function redactWin32DiagnosticPath(value, sandbox) {
  const text = String(value || "");
  const home = typeof sandbox?.hanakoHome === "string" ? sandbox.hanakoHome.trim() : "";
  if (!home) return text;
  return text.replace(new RegExp(escapeRegExp(home), "gi"), "<HANA_HOME>");
}

function emitWin32RuntimeFailureDiagnostic(onData, { kind, runtimeInfo, sandbox, exitCode }) {
  if (typeof onData !== "function") return;
  const executable = runtimeInfo?.shell || runtimeInfo?.git || runtimeInfo?.executable || "(unknown)";
  const lines = [
    "",
    `[win32-exec] ${kind} runtime failed before command output (STATUS_DLL_INIT_FAILED / 0xC0000142).`,
    `Exit code: ${exitCode}`,
    `Executable: ${redactWin32DiagnosticPath(executable, sandbox)}`,
    runtimeInfo?.bundledRoot ? `Runtime root: ${redactWin32DiagnosticPath(runtimeInfo.bundledRoot, sandbox)}` : null,
    sandbox?.hanakoHome ? "HANA_HOME: <HANA_HOME>" : null,
    `Route: explicit POSIX shell runtime`,
    "Default PowerShell/cmd/terminal execution is not changed by this diagnostic path.",
    "Likely causes: PortableGit DLL initialization failed inside the restricted-token sandbox, or the cached runtime is incomplete/blocked.",
    "Next step: clear HANA_HOME/.ephemeral/win32-sandbox-runtime and retry, then attach this diagnostic with the command log if it still fails.",
    "",
  ].filter(Boolean);
