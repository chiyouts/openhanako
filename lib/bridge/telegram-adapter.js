...
function telegramMessageOptions(options = {}) {
  const messageThreadId = options.messageThreadId ?? options.replyContext?.messageThreadId;
  if (messageThreadId == null || messageThreadId === "") return undefined;
  ...
  return {
    ...
    async sendReply(chatId, text, options = {}) {
      ...
      for (let i = 0; i < text.length; i += MAX) {
        ...
        if (messageOptions) await bot.sendMessage(chatId, chunk, messageOptions);
        else await bot.sendMessage(chatId, chunk);
      }
    },
